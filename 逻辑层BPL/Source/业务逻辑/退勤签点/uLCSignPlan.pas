...
begin
  ...
  strResult := Post('AddSignPlan',json.AsString);
  json.Clear();
...
begin
  ...
  strResult := Post('delsignplan',json.AsString);
  json.Clear();
  if not GetJsonResult(strResult,json,strErrMsg) then Exit;
  Result := True;
end;
...
begin
  ...
  strResult := Post('GetSignPlanList',json.AsString);
  json.Clear();
  if not GetJsonResult(strResult,json,strErrMsg) then Exit;
...
begin
  ...
  strResult := Post('GetSignPlanListByJiaoLuAry',json.AsString);
  if strErrMsg <> ''  then
  begin
    Exit;
  end;
...
begin
  ...
  signPlan.ToJson(iJsonPlan);
  json.o['data.SignPlan'] := iJsonPlan;
  strResult := Post('EditSignPlan',json.AsString);
  json.Clear();
  if not GetJsonResult(strResult,json,strErrMsg) then Exit;
  Result := True;
end;
...
begin
  ...
  json.O['data'] := iJson;
  strResult := Post('ModifySignPlanTrainman',json.AsString);
  json.Clear();
  iJson := nil;
  if not GetJsonResult(strResult,json,strErrMsg) then Exit;
  Result := True;
...
begin
  ...
  json.O['data'] := iJson;
  strResult := Post('ModifySignPlan',json.AsString);
  json.Clear();
  iJson :=nil;
  if not GetJsonResult(strResult,json,strErrMsg) then Exit;
  Result := True;
