...
type
  ...
  public
    //根据IP获取相应的岗位信息
    function GetSiteByRelationIP(IP : string;out RealIP:string;
    SiteJob : integer ;Site : TRsSiteInfo) : boolean;
    //
    function GetSiteByIP(IP : string; Site : TRsSiteInfo) : boolean;

    function GetSiteInfo(IP : string; Site : TRsSiteInfo) : boolean;
    //获取所有的SIte；
    procedure GetAllSites(out SiteArray: TRsSiteArray);
    //功能:获取全部的站点信息
...
procedure TRsDBSite.GetAllSites(out SiteArray: TRsSiteArray);
var
  i : integer;
  adoQuery : TADOQuery;
begin
  ...
  try
    with adoQuery do
    begin
      SQL.Text := 'Select * From TAB_Base_Site where ';
      Open;
...
function TRsDBSite.GetSiteInfo(IP: string; Site: TRsSiteInfo): boolean;
var
  strSql,strSiteGUID : string;
  adoQuery : TADOQuery;
begin
...
function TRsDBSite.GetSiteByRelationIP(IP: string; out RealIP:string ; SiteJob: integer;
  Site: TRsSiteInfo): boolean;
var
  strDstIP:string ;
  strSql,strSiteGUID : string;
  adoQuery : TADOQuery;
begin
  ...
  try
    ...
    begin
      ...
      strSql := 'select strDstSiteIP from Tab_Base_SiteRelation where strSrcSiteIP = %s and nToSiteJob = %d ';
      strSql := Format(strSql,[QuotedStr(IP),SiteJob]);
      SQL.Text := strSql;
      Open;


      if RecordCount = 0 then strDstIP := IP;


      strDstIP := Trim(FieldByName('strDstSiteIP').AsString);
      RealIP := strDstIP ;

      strSql := 'select * from TAB_Base_Site where strSiteIP = %s';
      strSql := Format(strSql,[QuotedStr(strDstIP)]);
      SQL.Text := strSql;
      Open;
