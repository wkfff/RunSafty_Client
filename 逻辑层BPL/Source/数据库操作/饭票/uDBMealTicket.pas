...
begin
  ...
  Result := InsertCanQuanInfo(MealTicket) ;
  Result := True ;
end;
...
function TRsDBMealTicket.Query(StartDate, EndDate: TDateTime; DriverCode,
...
begin
  ...
  try
    ...
    if ShenHeCode <> '' then
    begin
      strSql := strSql + ' and SHENHEREN_CODE = s' + QuotedStr(DriverCode);
    end;
    ...
    if sqlQuery.IsEmpty then
      Exit;
    SetLength(MealTicketList,sqlQuery.RecordCount);
    i := 0;
    while not sqlQuery.Eof do
    begin
      with MealTicketList[i] do
      begin
