...
type
  ...
  private
    //
    class procedure ADOQueryToData1(var Trainman: RRsTrainman;
  ...
  public
    ...
    //功能：修改乘务员
    procedure ModifyTrainman(TrainmanNumber : string;Trainman : RRsTrainman) ;
    //功能：删除乘务员
    ...
    //获取指定车间和人员交路内处于预备队列的人员信息
    procedure GetPrepareTrainmanByJiaoLu(WorkShopGUID : string ;TrainmanJiaoLu:string;out TrainmanArray : TRsTrainmanArray);
    //预备人员的状态转换成空
...
procedure TRsDBTrainman.GetPrepareTrainmanByJiaoLu(WorkShopGUID,
  TrainmanJiaoLu: string; out TrainmanArray: TRsTrainmanArray);
var
  i : integer;
  strSql : String;
  adoQuery : TADOQuery;
begin
  strSql :=  'select * from VIEW_Nameplate_TrainmanJiaolu_Prepare where nTrainmanState <> 7 and strWorkShopGUID = %s and strtrainmanjiaoluguid = %s order by strTrainmanNumber';
  strSql := Format(strSql,[QuotedStr(WorkShopGUID),QuotedStr(TrainmanJiaoLu)]);
  adoQuery := TADOQuery.Create(nil);
  try
    with adoQuery do
    begin
...
procedure TRsDBTrainman.ModifyTrainman(TrainmanNumber: string;
...
begin
  ...
  try
    with adoQuery do
    begin
      strSql := 'select * from TAB_Org_Trainman where  strTrainmanNumber = %s ';
      strSql := Format(strSql,[QuotedStr(TrainmanNumber)]);
      Connection := m_ADOConnection;
      Sql.Text := strSql;
      Open;
...
procedure TRsDBTrainman.QueryTrainmans_blobFlag(QueryTrainman: RRsQueryTrainman;
  PageIndex: integer; out TrainmanArray: TRsTrainmanArray;
  out nTotalCount: Integer);
var
  i : integer;
...
begin
  ...
  try
    ...
    begin
      ...
      while not eof do
      begin
        ADOQueryToData1(TrainmanArray[i],adoQuery,True);
        inc(i);
        next;
      end;
...
class procedure TRsDBTrainman.ADOQueryToData1(var Trainman: RRsTrainman;
