...
type
  ...
  //公寓叫班状态
  TRoomCallState = (TCS_Publish{生成计划},TCS_FIRSTCALL{首叫},TCS_RECALL{催叫},TCS_COMMIT{确认} );
  //公寓叫班类别
  TRoomCallType =(TCT_AutoCall{自动叫班},TCT_MunualCall{人工叫班});
  //叫班状态
  ...
  {公寓叫班结果类型}
  TRoomCallResult =(TR_SUCESS{成功},TR_FAIL{失败},TR_CANCEL{取消},TR_TIMEOUT{超时});
const
  TRsPlanStateNameAry : array[TRsPlanState] of string =
    ('已取消','编辑','已下发','已接收','已发布','已入寓','已离寓',
    '已出勤','已退勤','异常结束');
  ...
  TRsPlanStateColorAry : array [TRsPlanState] of TColor =
    (clGray,clWhite,clRed,clYellow,clFuchsia,clAqua,clScrollBar,clWindow,clSilver,clRed);
  ...
  TRoomCallStateColorAry : array[TRoomCallState] of TColor  =
          (clAqua{生成计划},clScrollBar{首叫},clWindow{催叫},clGreen);
  ...
  TRoomCallResultNameAry:array[TRoomCallResult] of string =
      ...
      {职务类型}
  TRsPostName : array[TRsPost] of string =
  ('无','司机','副司机','学员');
