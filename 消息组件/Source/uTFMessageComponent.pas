...
type
  ...
  private
    ...
    function CreateConfirmURL_StrParams(strParams: string): string;
    {功能:消息解析}
      ...
      begin
        ...
        begin
          try
            ...
            begin
              strMsg := 'HttpRecieveMessage';
              HttpRecieveMessage();
              strMsg := 'NotifyMessages';
              NotifyMessages();
              strMsg := 'ConfirmMessages';
...
begin
  ...
  Result :=
    TIdURI.URLEncode(AnsiToUtf8( Trim(m_strURLHost) + 'clientID=' + m_strClientID + '&mode=' +
    inttostr(Ord(hmConfirmMessage)) + '&msgID=' + strMsgIDParam));

end;

function TMsgDataComm.CreateConfirmURL_StrParams(strParams: string): string;
begin
  Result :=
    TIdURI.URLEncode(AnsiToUtf8( Trim(m_strURLHost) + 'clientID=' + m_strClientID + '&mode=' +
    inttostr(Ord(hmConfirmMessage)) + '&msgID=' + strParams));
end;
