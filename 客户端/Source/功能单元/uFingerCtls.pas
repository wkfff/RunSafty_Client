...
uses
  ...
  Windows,uHttpWebAPI,Contnrs,Messages,Forms;
//******************************************************************************
//    指纹仪控制单元，实现指纹后台下载及指纹缓存管理
//******************************************************************************
const
  FINGERPATH = 'Fingers\';
  WM_FINGERUPDATE = WM_USER + 2023;
type
  ...
  TFingerLoadBuf = class
  private
    m_FingerIdentity: string;
    m_CurrentID: integer;
    m_TmLst: TList;
  public
    constructor Create();
    destructor Destroy;override;
    procedure Clear();
    procedure AddFinger(Tm: PTm);
    property CurrentID: integer read m_CurrentID write m_CurrentID;
    property TmLst: TList read m_TmLst;
    property FingerIndentity: string read m_FingerIdentity write m_FingerIdentity;
  end;
  ...
  public
    ...
    procedure LoadFingers(Buf: TFingerLoadBuf);
  ...
  public
    ...
    procedure SetSyncLogCallBack(CallBack: TOnEventByString);
  ...
  function HiDWord(v: Double): integer;
  function LowDWord(v: Double): integer;
  function ComposeDWord(hiBytes,lowBytes: integer): Double;
...
function LowDWord(v: Double): integer;
begin
  Move(PByte(integer(@v) + 4)^,result,4);
end;

function ComposeDWord(hiBytes,lowBytes: integer): Double;
begin
  Move(hiBytes,Result,4);
  Move(lowBytes,PByte(integer(@Result) + 4)^,4);
end;
...
procedure TServerFingerCtl.LoadFingers(Buf: TFingerLoadBuf);
var
  nStartID: integer;
  nTotalCount: integer;
  TempArray: TRsTrainmanArray;
...
begin
  ...
  repeat
    nStartID := Buf.CurrentID;
    m_RsLCTrainmamMgr.GetTrainmansBrief(nStartID,10,1,TempArray,nTotalCount);

    for I := 0 to Length(TempArray) - 1 do
    begin
      Buf.AddFinger(PTm(@TempArray[i]));
    end;
  until Length(TempArray) = 0;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        WriteLog(Format('指纹识别成功 [%s]',[tmNumber]));
...
begin
  ...
  t := Now;
  PostMessage(Application.MainFormHandle,WM_FINGERUPDATE,HiDWord(t),LowDWord(t));
end;
...
procedure TFingerPrintCtl.SetSyncLogCallBack(CallBack: TOnEventByString);
begin
  m_CallBack := CallBack;
  m_SynFingerLoader.OnLogOut := CallBack;
end;
...
procedure TFingerPrintCtl.WriteLog(const log: string);
begin
...
var
  ...
  Buf: TFingerLoadBuf;
begin
  Buf := TFingerLoadBuf.Create;
  try
    while not m_Thread.Terminated do
    begin

      try
        ThreadDelay(m_nInterval div 2,m_Thread);
        ...
        begin
          ...
          begin
            if Buf.FingerIndentity <> m_ServerFingerID then
            begin
              Buf.FingerIndentity := m_ServerFingerID;
              Buf.Clear();
            end;

            m_ServerFingerCtl.LoadFingers(Buf);

            SetLength(m_TmArray,Buf.m_TmLst.Count);

            for I := 0 to Buf.m_TmLst.Count - 1 do
            begin
              m_TmArray[i] := PTm(Buf.TmLst[i])^;
              TLocalFingerCtl.SaveFingerFile(m_AppPath + FINGERPATH,PTm(Buf.TmLst[i])^);
            end;
...
begin
 ...
 { TFingerLoadBuf }
procedure TFingerLoadBuf.AddFinger(Tm: PTm);
var
  newTm: PTm;
begin
  New(newTm);
...
procedure TFingerLoadBuf.Clear;
var
  tm: PTm;
begin
  while m_TmLst.Count > 0 do
  begin
    tm := PTm(m_TmLst.Last);
    Dispose(tm);
    m_TmLst.Delete(m_TmLst.Count - 1);
  end;
...
constructor TFingerLoadBuf.Create;
begin
  Inherited;
  m_TmLst := TList.Create;
end;

destructor TFingerLoadBuf.Destroy;
begin
  Clear();
  m_TmLst.Free;
  inherited;
