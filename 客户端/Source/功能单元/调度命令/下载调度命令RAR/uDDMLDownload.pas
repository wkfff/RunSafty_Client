...
uses
  ...
  IdFTPList,uGlobalDM,Dialogs,RAR;
const
   ...
   // if FTPTransportControl.CheckFileExists('','调度命令.rar') = False then Exit ;
    ...
    begin
     // strList.Add(IdFTPListItems.Items[i].FileName);
      ...
      begin
        if FormatDateTime('yyyy-mm-dd hh:mm:ss',GlobalDM.dtDLRARUpdate) <>
...
begin
  ...
  try
    ...
    begin
      bUpdated := True;
      GlobalDM.dtDLRARUpdate := dtNewRarFile;
      Result := True;
    end;
  except on e: Exception do
...
begin
  ...
  strServerFileName := GlobalDM.AppPath + DDMLFile_DIRName +  CS_DDMLFILENAME;
...
begin
  FTPTransportControl.FTPConfig := GlobalDM.DLFTPConfig;
end;
...
begin
  ...
  s_pathName := GlobalDM.AppPath + DDMLFile_DIRName +'调度命令.RAR';
  d_pathName := GlobalDM.AppPath + DDMLFile_DIRName;
