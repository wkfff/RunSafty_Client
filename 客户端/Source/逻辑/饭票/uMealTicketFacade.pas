...
type
  ...
  public
    ...
    function IsGivedTicket(TMNumber:string;Plan : RRsTrainmanPlan;var CanQuanA : integer;
      var CanQuanB : integer; var CanQuanC : integer) : boolean;virtual;
        //修改餐券信息
    function ModifyMealTicket(TrainmanNumber:string;TrainmanPlan:RRsTrainmanPlan;CanQuanA,CanQuanB,CanQuanC:Integer):boolean;virtual;  
  end;
  ...
  public
    ...
    function IsGivedTicket(TMNumber:string;Plan : RRsTrainmanPlan;var CanQuanA : integer;
      var CanQuanB : integer; var CanQuanC : integer) : boolean;override;

    function ModifyMealTicket(TrainmanNumber:string;TrainmanPlan:RRsTrainmanPlan;CanQuanA,CanQuanB,CanQuanC:Integer):boolean;override;
...
uses utfPopBox,uLCWebAPI,uMealTicketConfig, uGlobalDM,uMealTicketRule,
  ufrmTicketCountInput;
...
begin
  ...
  begin
    TtfPopBox.ShowBox('发放饭票错误: '+strError,2000);
    exit;
  end;
...
begin
  ...
  begin
    TtfPopBox.ShowBox('发放饭票错误: '+strTxt,2000);
    exit;
  end;
  //发放饭票
...
function TTsMealTicket.IsGivedTicket(TMNumber: string; Plan: RRsTrainmanPlan;
  var CanQuanA, CanQuanB, CanQuanC: integer): boolean;
var
  strError: string;
begin
  result := false;
  ...
  result := m_ticketManager.IsGivedTicket(TMNumber,Plan,CanQuanA,CanQuanB,CanQuanC);
end;
...
function TTsMealTicket.ModifyMealTicket(TrainmanNumber: string;
...
begin
  ...
  result := m_ticketManager.ModifyMealTicket(TrainmanNumber,TrainmanPlan,
...
function TMealTicket.IsGivedTicket(TMNumber:string;Plan : RRsTrainmanPlan;var CanQuanA : integer;
      var CanQuanB : integer; var CanQuanC : integer) : boolean;
begin
  result := false;
end;

function TMealTicket.ModifyMealTicket(TrainmanNumber: string;
  TrainmanPlan: RRsTrainmanPlan; CanQuanA, CanQuanB, CanQuanC: Integer):boolean;
begin
  result := false;
end;
...
begin
  ...
  if not TMealCfg.TicketEnable[GlobalDM.SiteInfo.WorkShopGUID] then
  begin
    Result := TNoneTicket.Create(nil,nil,'')
  end
  else
  begin
    TicketVersion := GlobalDM.DB_SysConfig('TicketVersion',GlobalDM.SiteInfo.strSiteIP);
    if TicketVersion = 'TFMEALTICKET' then
    begin
      Result := TTFMealTicket.Create(GlobalDM.LogManage.InsertLog,GlobalDM.WebAPIUtils,
      GlobalDM.SiteInfo.WorkShopGUID);

      Result.m_Active := GlobalDM.UsesMealTicket;
    end
    else
    begin
      Result := TTsMealTicket.Create(GlobalDM.LogManage.InsertLog,GlobalDM.WebAPIUtils,
      GlobalDM.SiteInfo.WorkShopGUID);
      Result.m_Active := GlobalDM.UsesMealTicket;
      (Result as TTsMealTicket).NumberLen := GlobalDM.TicketNumberLen;

      (Result as TTsMealTicket).m_ticketManager.UseMealCheJian :=
          GlobalDM.DB_SysConfig('MealTicket','UseMealCheJian') = '1';
    end;
  end;
...
function TTFMealTicket.IsGivedTicket(TMNumber: string; Plan: RRsTrainmanPlan;
  var CanQuanA, CanQuanB, CanQuanC: integer): boolean;
var
  mealInfo : TTFMealTicketEntity;
begin
  mealInfo := TTFMealTicketEntity.Create;
  try
    result := m_RsLCTFMealTicket.Get(TMNumber,mealInfo);
    if result then
    begin
      CanQuanA := mealInfo.nCanQuanA;
      CanQuanB := mealInfo.nCanQuanB;
      CanQuanC := 0;
    end;
...
function TTFMealTicket.ModifyMealTicket(TrainmanNumber: string;
  TrainmanPlan: RRsTrainmanPlan; CanQuanA, CanQuanB,
  CanQuanC: Integer): boolean;
begin
  raise Exception.Create('未实现本功能');
end;
...
begin
  Result :=
    GlobalDM.DB_SysConfig('MealTicket','ManualSend_' + WorkShopID) <> '0';
end;

class function TMealCfg.GetTicketEnable(const WorkShopID: string): Boolean;
begin
  Result := GlobalDM.DB_SysConfig('MealTicket','TicketEnable_' + WorkShopID) <> '0';
end;

class procedure TMealCfg.SetManualSend(const WorkShopID: string;const Value: Boolean);
begin
  if Value then
    GlobalDM.SetDBSysConfig('MealTicket','ManualSend_' + WorkShopID,'1')
  else
    GlobalDM.SetDBSysConfig('MealTicket','ManualSend_' + WorkShopID,'0');
end;
...
begin
  if Value then
    GlobalDM.SetDBSysConfig('MealTicket','TicketEnable_' + WorkShopID,'1')
  else
    GlobalDM.SetDBSysConfig('MealTicket','TicketEnable_' + WorkShopID,'0');
end;
