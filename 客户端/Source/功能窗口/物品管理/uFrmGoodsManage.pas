...
uses
  ...
  ComCtrls, RzCmboBx,ADODB,uLendingDefine,ufrmTrainmanIdentity,
  uSaftyEnum, Menus,DateUtils,uWorkShop, RzDTP, Mask, RzEdit,
  RzCommon, RzStatus,uFrmGoodsRangeManage,uLCGoodsMgr;
const
...
uses uFrmGoodsSend, uTrainman, ufrmGoodsQuery, uGlobalDM;
...
begin
  ...
  try
    ...
    Condition.strWorkShopGUID := GlobalDM.SiteInfo.WorkShopGUID;
...
procedure TfrmGoodsManage.OnFingerTouching(Sender: TObject);
var
  TrainMan: RRsTrainman;
...
begin
  ...
  if SendLendings(TrainMan,Verify) then
  begin
    btnQuery.Click;
  end;
end;
