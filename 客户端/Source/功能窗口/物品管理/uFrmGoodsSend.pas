...
uses
  ...
  utfPopBox,Contnrs,uLCGoodsMgr,uLCTrainmanMgr;
  ...
  try
    ...
    begin
      try
        ...
        begin
          SendGoods();
          TtfPopBox.ShowBox('发放成功!');
        end
        else
        begin
          GiveBackGoods();
          TtfPopBox.ShowBox('回收成功!');
        end;
        ModalResult := mrOk;
      except
        on E: Exception do
...
begin
  ...
  try
    ...
    begin
      ...
      //检查物品是否在发放范围内
      if GlobalDM.UsesGoodsRange then
      begin
...
begin
  ...
  if GlobalDM.DutyUser.strDutyNumber <> '1' then
  begin
    if m_RsLCTrainmanMgr.GetTrainmanByNumber('230' + GlobalDM.DutyUser.strDutyNumber,Trainman) then
    begin
      m_SelectLenderGUID := Trainman.strTrainmanGUID;
...
begin
  ...
  try
    ...
    m_RsLCGoodsMgr.Send(m_SelectTrainmanGUID,GlobalDM.SiteInfo.WorkShopGUID,
    edtRemark.Text,GlobalDM.UsesGoodsRange,LendingDetailList);
  finally
