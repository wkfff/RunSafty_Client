...
uses
  ...
  Dialogs, StdCtrls,uGlobalDM,uTrainman,uTFSystem ,uDrink,
...
begin
  ...
  dtStartTime := incHour(GlobalDM.GetNow,-2);
  SetLength(m_drinkAry,8);
  FillChar(m_drinkAry[0],Length(m_drinkAry) * SizeOf(RRsDrink),#0);

  if m_RsLCDrink.GetTrainNoDrinkInfo(dtStartTime,strTrainNo,GlobalDM.DutyPlace.placeID,
...
//  if strTrainNo = '' then Exit;
  ...
  if m_RsLCDrink.GetTMLastDrinkInfo(GlobalDM.SiteInfo.strSiteIP,strTMNumber,dtStartTime,drinkInfo,strErr)= False then
  begin
    Box(strErr);
    Exit;
  end;
