...
uses
  ...
  RzPanel,uMealTicket,utfsystem,uDBMealTicket,StrUtils;
...
var
  error: string;
begin
  if not GlobalDM.UsesMealTicket then
  begin
    Box('未启用饭票功能!');
    Exit;
  end;
  if not GlobalDM.ConnectMealDB(error) then
  begin
    Box(error);
    Exit;
  end;
...
begin
  ...
  m_RsDBMealTicket := TRsDBMealTicket.Create(GlobalDM.TicketConnection);
end;
...
begin
  ...
  if (GlobalDM.TicketNumberLen < 7) and (Length(strDriver) >= GlobalDM.TicketNumberLen) then
    strDriver := RightStr(strDriver,GlobalDM.TicketNumberLen);
