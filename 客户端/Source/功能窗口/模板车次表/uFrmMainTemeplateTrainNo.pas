...
//    procedure SelectGroup(GroupID:Integer);
    //高亮指定GUID的的行
//    function SelectDayPlan(StringGrid:TAdvStringGrid;DayPlanGUID:string):Boolean;
  private
  //是否修改CELLS的值
...
implementation
uses
  uGlobalDM,ufrmHint ,utfPopBox, uDayPlanExportToXls;
...
begin
  ...
  begin
    ...
    dtCreateTime:= GlobalDM.GetNow ;  //产生事件
    strTrainNo1:= '' ;     //车次1
    strTrainInfo:= '' ;    //机车信息
    strTrainNo2:= '';      //车次2
    strTrainNo:= '' ;      //真正的车次信息
...
begin
  ...
  TfrmHint.ShowHint('正在从模板机车计划表中加载计划,请稍等...');
  try
    try
...
begin
  ...
  try
    ...
    begin
      if not m_webTrainPlan.CancelTrainPlan(CancelPlanList, GlobalDM.DutyUser.strDutyGUID,MAIN_PLAN,strError) then
      begin
        Box(strError);
        exit;
      end
...
//
//function TFrmMainTemeplateTrainNo.FormatCC(SourceString: string): string;
//var
//  i: Integer;
//  b,e : integer;
//begin
...
procedure TFrmMainTemeplateTrainNo.FormCreate(Sender: TObject);
var
...
//procedure TFrmMainTemeplateTrainNo.OnProgress(nCompleted, nTotal: integer);
//begin
...
begin
  ...
  RsLCBaseDict.LCTrainJiaolu.GetTrainJiaoluArrayOfSite(GlobalDM.SiteInfo.strSiteGUID,TrainjiaoluArray);
