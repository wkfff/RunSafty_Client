...
uses
  ...
  uApparatusCommon, RzPanel, RzTabs, PngSpeedButton,uTFVariantUtils,uGlobalDM,
  ZKFPEngXUtils;
...
type
  ...
  {功能:饮酒测试}
  procedure TestDrinking(Trainman:RRsTrainman;TrainTypeName,TrainNumber,TrainNo:string;
...
begin
  ...
  try
    ...
    TestAlcoholInfo.nAlcoholicity := frmTestDrinking.m_TestAlcoholInfo.nAlcoholicity ;
...
begin
  ...
  Picture.Canvas.TextOut(3,60,'时间:'+formatDateTime('hh:mm:ss',
      m_TestAlcoholInfo.dtTestTime));
  Picture.Canvas.TextOut(3,79,'酒精:' + IntToStr( m_TestAlcoholInfo.nAlcoholicity));
...
begin
  ...
  begin
    ...
    if GlobalDM.IsUseLocalDrinkSound then
      PlaySound(Pchar(strFile), 0,SND_FILENAME or SND_ASYNC);
  end;
end;
...
begin
  ...
  m_TestAlcoholInfo.nAlcoholicity := dwAlcoholicity ;
  ms := TMemoryStream.Create;
  try
    if not m_bCaptured then
    begin
