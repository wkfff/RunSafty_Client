...
var
  frmUserInfoEditAccess: TfrmUserInfoEditAccess;
...
class function TfrmUserInfoEditAccess.AppendTrainmanInfo: Boolean;
//功能:添加乘务员信息
var
  frmUserInfoEdit: TfrmUserInfoEditAccess;
begin
  Result := False;
  frmUserInfoEdit:= TfrmUserInfoEditAccess.Create(nil);
  try
...
procedure TfrmUserInfoEditAccess.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel ;
end;

procedure TfrmUserInfoEditAccess.btnCapturePictureClick(Sender: TObject);
var
...
procedure TfrmUserInfoEditAccess.btnFingerRegisterClick(Sender: TObject);
begin
  if FingerRegister(m_strFinger1Register,m_strFinger2Register,GlobalDM.ZKFPEngX) then
    m_bChange := True;
end;

procedure TfrmUserInfoEditAccess.btnLoadPictureClick(Sender: TObject);
begin
...
procedure TfrmUserInfoEditAccess.btnSaveClick(Sender: TObject);
var
  nID:Integer ;
...
function TfrmUserInfoEditAccess.CheckInput: Boolean;
//功能:检查用户输入是否合法
begin
  Result := False;
  ...
  if m_DBTrainman.ExistTrainmanByNumber( Trim(edtNumber.Text)) then
  begin
    Box('该工号已经存在！请重新输入！');
    edtNumber.SetFocus;
    edtNumber.SelectAll;
...
procedure TfrmUserInfoEditAccess.FormCreate(Sender: TObject);
begin
...
procedure TfrmUserInfoEditAccess.FormDestroy(Sender: TObject);
begin
  m_DBTrainman.Free;
  m_PictureStream.Free;
end;

procedure TfrmUserInfoEditAccess.FormShow(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmUserInfoEditAccess.GatherData(var trainman: RRsTrainman);
var
...
procedure TfrmUserInfoEditAccess.InitOrgCtrls;
var
  i : integer;
...
procedure TfrmUserInfoEditAccess.InitZKFPEng;
{功能:初始化指纹仪}
begin
...
class function TfrmUserInfoEditAccess.ModifyTrainmanInfo(
...
procedure TfrmUserInfoEditAccess.ReadData(strGUID: String);
//功能:读取乘务员信息
var
  PictureStream : TMemoryStream;
...
procedure TfrmUserInfoEditAccess.WMDevicechange(var Msg: TMessage);
begin
