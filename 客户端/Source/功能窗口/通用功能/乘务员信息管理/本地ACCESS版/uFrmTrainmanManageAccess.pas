...
uses
  ...
  uGlobalDM,uSaftyEnum,uTrainman,uBaseDBRoomSign,uDBAccessRoomSign,utfsystem,
  ComObj,uWorkShop,uGuideGroup,uTrainJiaolu;

type
  TfrmTrainmanManageAccess = class(TForm)
    Panel1: TPanel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
var
  frmTrainmanManageAccess: TfrmTrainmanManageAccess;

implementation

uses
  uFrmProgressEx ,
  ufrmUserInfoEditAccess;

{$R *.dfm}

procedure TfrmTrainmanManageAccess.btnAppendTrainmanClick(Sender: TObject);
begin
  if not TfrmUserInfoEditAccess.AppendTrainmanInfo then
  begin
    exit;
  end;
  btnQuery.Click;
...
procedure TfrmTrainmanManageAccess.btnDeleteTrainmanClick(Sender: TObject);
var
...
procedure TfrmTrainmanManageAccess.btnExportClick(Sender: TObject);
begin
  ExportTrainmans;
end;

procedure TfrmTrainmanManageAccess.btnImportClick(Sender: TObject);
begin
  ImportTrainmans;
end;

procedure TfrmTrainmanManageAccess.btnModifyTrainmanClick(Sender: TObject);
var
  strTrainmanGUID : string;
begin
    if length(m_TrainmanArray) = 0 then
  ...
  strTrainmanGUID := strGridTrainman.Cells[99,strGridTrainman.Row];
  if not TfrmUserInfoEditAccess.ModifyTrainmanInfo(strTrainmanGUID) then
  begin
    exit;
  end;
  btnQuery.Click;
...
procedure TfrmTrainmanManageAccess.btnQueryClick(Sender: TObject);
begin
  QueryTrainmans;
end;

procedure TfrmTrainmanManageAccess.comboWorkShopChange(Sender: TObject);
begin
  Exit ;
  InitGuideGroup();
  InitTrainJiaoLu();
...
procedure TfrmTrainmanManageAccess.edtTrainmanNumberKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
    btnQueryClick(nil);
...
procedure TfrmTrainmanManageAccess.ExportTrainmans;
var
...
procedure TfrmTrainmanManageAccess.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action := caFree;
  frmTrainmanManageAccess := nil;
end;

procedure TfrmTrainmanManageAccess.FormCreate(Sender: TObject);
begin
...
procedure TfrmTrainmanManageAccess.FormDestroy(Sender: TObject);
begin
  m_DBTrainman.Free;

end;

procedure TfrmTrainmanManageAccess.FormShow(Sender: TObject);
begin
...
procedure TfrmTrainmanManageAccess.ImportTrainmans;
var
  excelApp: Variant;
...
procedure TfrmTrainmanManageAccess.Init;
var
  i : integer;
...
procedure TfrmTrainmanManageAccess.InitGuideGroup;
var
...
procedure TfrmTrainmanManageAccess.InitTrainJiaoLu;
var
...
class procedure TfrmTrainmanManageAccess.OpenTrainmanQuery(bQuery:Boolean = False);
begin
  if frmTrainmanManageAccess = nil then
  begin
    frmTrainmanManageAccess :=  TfrmTrainmanManageAccess.Create(nil);
    frmTrainmanManageAccess.bIsQueryForm := bQuery;
    frmTrainmanManageAccess.Init;
    frmTrainmanManageAccess.Show;
  end
  else
  begin
    frmTrainmanManageAccess.bIsQueryForm := bQuery;
    frmTrainmanManageAccess.Show;
    frmTrainmanManageAccess.WindowState := wsMaximized;
  end;
end;

procedure TfrmTrainmanManageAccess.QueryTrainmans;
var
