...
type
  ...
  private
    ...
    //初始化车队
    procedure InitGuideGroups();
    //初始化区段
    procedure InitTrainjiaolus();
    procedure SetEditType(const Value: TDBOperationType);
  public
    { Public declarations }
    //信息编辑状态
    property EditType : TDBOperationType read m_EditType  write SetEditType;
  end;
  {功能:添加乘务员信息}
...
begin
  ...
  try
    ...
    frmUserInfoEdit.EditType := otInsert;
...
begin
  ...
  try
    ...
    frmUserInfoEdit.EditType := otModify;    
    frmUserInfoEdit.InitOrgCtrls;
...
procedure TfrmUserInfoEdit.InitGuideGroups;
var
...
begin
  ...
  selectedIndex := comboArea.Values.IndexOf(LeftStr(GlobalDM.SiteInfo.strSiteIP,2));
  if selectedIndex > -1 then
  begin
    comboArea.ItemIndex := selectedIndex;
  end;
end;
{功能:初始化车间}
...
begin
  comboArea.OnChange := nil;
  comboWorkShop.OnChange := nil; 
  try
    InitJWD();
    InitWorkShop;
    InitTrainjiaolus;
    ...
    comboArea.OnChange := comboAreaChange;
    comboWorkShop.OnChange := comboWorkShopChange;
  end;
end;

procedure TfrmUserInfoEdit.InitTrainjiaolus;
var
...
procedure TfrmUserInfoEdit.SetEditType(const Value: TDBOperationType);
begin
  m_EditType := value;
  comboArea.Enabled := false;
  comboWorkShop.Enabled := false;
  comboTrainJiaolu.Enabled := false;
  comboGuideGroup.Enabled := false;
  if m_EditType = otInsert then
  begin
    comboArea.Enabled := true;
    comboWorkShop.Enabled := true;
    comboTrainJiaolu.Enabled := true;
    comboGuideGroup.Enabled := true;
  end;
