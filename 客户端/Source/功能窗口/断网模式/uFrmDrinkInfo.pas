...
uses
  ...
  uTFSystem, uConnAccess, uSaftyEnum, uTrainman, uDrink, uLCDrink;
...
type
  ...
  private
    ...
    //查询本地测酒信息，nWorkTypeID=2出勤 3退勤
    procedure QueryLocalDrinkInfo(DrinkQuery: RRsDrinkQuery);   
...
procedure TFrmDrinkInfo.FormCreate(Sender: TObject);
begin
  m_dbConnAccess := TConnAccess.Create(Application);
  m_RsLCDrink := TRsLCDrink.Create(GlobalDM.WebAPIUtils);
end;

procedure TFrmDrinkInfo.FormDestroy(Sender: TObject);
begin          
  m_RsLCDrink.Free;
  m_dbConnAccess.Free;
end;
...
begin
  ...
  begin
    ...
    QueryLocalDrinkInfo(DrinkQuery);
  end;
...
procedure TFrmDrinkInfo.QueryLocalDrinkInfo(DrinkQuery: RRsDrinkQuery);
var
  i: integer;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Cells[1, i + 1] := Format('%6s[%s]', [DrinkInfoArray[i].strTrainmanName, DrinkInfoArray[i].strTrainmanNumber]);
        Cells[2, i + 1] := TestAlcoholResultToString(TTestAlcoholResult(DrinkInfoArray[i].nDrinkResult));
        Cells[3, i + 1] := FormatDateTime('yyyy-mm-dd hh:nn:ss',DrinkInfoArray[i].dtCreateTime);  
        Cells[4, i + 1] := TRsRegisterFlagNameAry[TRsRegisterFlag(DrinkInfoArray[i].nVerifyID)];
        Cells[5, i + 1] := DrinkInfoArray[i].strDutyNumber;
        Cells[999, i + 1] := IntToStr(DrinkInfoArray[i].nDrinkInfoID);
      end;
    end;
...
begin
  try
    m_RsLCDrink.QueryNoPlanDrink(DrinkQuery.dtBeginTime, DrinkQuery.dtEndTime,
...
begin
  ...
  if m_DrinkFrom = dfLocal then
  begin
    result := m_dbConnAccess.GetDrinkInfo(StrToIntDef(m_strDrinkInfoID,0), DrinkInfo);
  end;

  if m_DrinkFrom = dfServer then
  begin
    if m_RsLCDrink.GetDrinkInfo(m_strDrinkInfoID, Drink) then
    begin
