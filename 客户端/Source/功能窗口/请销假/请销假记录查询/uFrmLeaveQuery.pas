...
uses
  ...
  uLeaveListInfo, uGlobalDM, uTFSystem, uFrmAskLeaveDetail, RzCmboBx, ComObj,
  Mask, RzEdit, uTrainman, uGuideGroup, RzButton, RzRadChk,
  uLCAskLeave,uLCTeamGuide;
...
implementation
uses
  DateUtils,uFrmAskLeave,uFrmCancelLeave,uFrmProgressEx,uGuideSign;
{$R *.dfm}
...
begin
  if not TFrmAskLeave.ShowAskLeaveForm then exit;
  QueryLeaveInfo;
end;
...
begin
  ...
  if not TFrmCancelLeave.ShowForm then exit;
  QueryLeaveInfo;
end;
...
procedure TFrmLeaveQuery.FormCreate(Sender: TObject);
begin
...
begin
  ...
  //添加指导队信息
  m_RsLCGuideGroup.GetGroupArray(GlobalDM.SiteInfo.WorkShopGUID, GuideGroupArray);
