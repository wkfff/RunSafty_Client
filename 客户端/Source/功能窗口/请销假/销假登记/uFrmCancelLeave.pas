...
begin
  ...
  //验证该职工是否是本车间人员
  if m_Trainman.strWorkShopGUID <> GlobalDM.SiteInfo.WorkShopGUID then
  begin
  ...
  CancelLeaveDetail.strDutyUserID := GlobalDM.DutyUser.strDutyGUID;
  CancelLeaveDetail.strDutyUserName := GlobalDM.DutyUser.strDutyName;
  CancelLeaveDetail.strSiteID := GlobalDM.SiteInfo.strSiteGUID;
  CancelLeaveDetail.strSiteName := GlobalDM.SiteInfo.strSiteName;
...
class function TFrmCancelLeave.ShowForm : boolean;
var
  frmForm: TFrmCancelLeave;
begin
  result := false;
...
procedure TFrmCancelLeave.FormCreate(Sender: TObject);
begin
