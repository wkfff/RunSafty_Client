...
uses
  ...
  uLCTrainmanMgr,uFrmKeyTrainmanView,uFrmFixedGroupView;
const
...
type
  TfrmNameBoardManage = class(TForm)
    ...
    procedure mniNamedViewKeyTrainmanClick(Sender: TObject);
    procedure mniNamedViewFixedGroupClick(Sender: TObject);
  ...
  public
    { Public declarations }
    class procedure OpenNameBoardManage(EditEnabled : boolean = true);
    class procedure EnableDelPlate(Value: Boolean);
  end;
...
implementation
uses
  uGlobalDM,uFrmAddGroup,uFrmAddCheCi,uFrmAddUser,uFrmAddJiChe,DateUtils,uFrmTrainmanSelect,
  ufrmHint,ComObj,uFrmProgressEx,
  uFrmPlanInfo,uFrmNameBorardSelectStation,uFrmSelectDutyPlace,uLCBaseDict,
  uGroupXlsExporter, uDutyUser;
var
...
begin
  ...
  try
    ...
    //显示预备人员
//    if FindRet.nTrainmanState in [Ord(tsReady), Ord(tsNil)] then
//    begin
//      Node := treeJiaolu.Items[0];
//      Node.Expanded := true;
//      if not Node.Selected then
//        treeJiaolu.Select(Node)
//      else
//        InitJiaoluTab;
//      exit;
//    end;
    //显示非运转人员
//    if FindRet.nTrainmanState = Ord(tsUnRuning) then
//    begin
//      Node := treeJiaolu.Items[1];
//      Node.Expanded := true;
//      if not Node.Selected then
//        Node.Selected := true
//      else
...
class procedure TfrmNameBoardManage.EnableDelPlate(Value: Boolean);
begin
  if Assigned(frmNameBoardManage) then
  begin
    frmNameBoardManage.mniDelPlate.Visible := Value;
  end;
end;
...
begin
  ...
  m_webTrainJiaoLu := TRsLCTrainJiaolu.Create(GlobalDM.GetWebUrl,GlobalDM.SiteInfo.strSiteIP,GlobalDM.SiteInfo.strSiteGUID);
...
//  tabtopBoard.BeginUpdate;
  ...
  TfrmHint.ShowHint('正在加载名牌数据……');
  try
    if treeJiaolu.Selected = nil then
    begin
      tabZFQD.Tabs.Clear;
      tabtopBoard.ClearView;
      GlobalDM.NameplateTrainmanJiaolu := '';
      exit;
    end;
    m_SelectedTrainmanJiaolu := m_TrainmanJiaoluArray[Integer(treeJiaolu.Selected.Data)];
    GlobalDM.NameplateTrainmanJiaolu := m_SelectedTrainmanJiaolu.strTrainmanJiaoluGUID;
...
begin
  ...
  try
    ...
    begin
      //if not IsQueriedGroup(namedGroupArray[i].Group) then continue;
      ...
      //if (Trim(edtTrainmanNumber.Text) <> '') or (Trim(edtTrainmanName.Text) <> '') or (Trim(edtPY.Text) <> '') then
      begin
...
begin
  ...
  try
    ...
    begin
      //if not IsQueriedTrainman(trainmanArray[i]) then continue;
...
begin
  ...
  begin
    ...
    try
      RsLCBaseDict.LCTrainmanJiaolu.GetTMJLByTrainJL(trainjiaoluGUID,trainmanJiaoluArray);
    except
      on E: Exception do
      begin
        BoxErr(E.Message);
    ...
    begin
      ...
      if length(GlobalDM.NameplateTrainmanJiaolu) > 1 then
      begin
        if (m_TrainmanJiaoluArray[length(m_TrainmanJiaoluArray) -1].strTrainmanJiaoluGUID = GlobalDM.NameplateTrainmanJiaolu)
...
begin
  ...
  try
    ...
    for i := 0 to length(trainmanArray) - 1 do
    begin
      //if not IsQueriedTrainman(trainmanArray[i].Trainman) then continue;
      if i = 0 then
      begin
...
begin
  ...
  try
    begin
      ...
      for i := 0 to length(orderGroupArray) - 1 do
      begin
        //if not IsQueriedGroup(orderGroupArray[i].Group) then continue;
        orderView := TOrderGroupView.Create(nil);
...
begin
  ...
  try
    ...
    tabtopBoard.AddView(namedView);
    ReorderNamedViews;
    tabtopBoard.VertScrollBar.Position := namedView.Top;
    tabtopBoard.ReCanvas;
  except on  e : exception do
    begin
      Box('添加失败：' + e.Message);
...
begin
  ...
  try
    ...
    tabtopBoard.VertScrollBar.Position := orderView.Top;
    tabtopBoard.ReCanvas;
  except on  e : exception do
    begin
      Box('添加失败：' + e.Message);
...
begin
  {if not TFrmAddUser.InputTrainman(m_strWorkShopGUID,trainman) then exit;}
  if not TFrmAddUser.InputTrainman('',trainman) then exit;
...
//  if namedView.Parent is not TView then
//    exit ;
...
begin
  ...
  try
    {
    tempOrderGroupVIEW := TOrderGroupView.Create(nil);
    tempOrderGroupVIEW.OrderGroup := sourceVIEW.OrderGroup;
    tempOrderGroupVIEW.Images := PngImageCollection;    
    tabtopBoard.ViewList.Remove(sourceVIEW);
    if nextOrderView <> nil then
    begin
      nIndex := tabtopBoard.ViewList.IndexOf(nextOrderView);
      tabtopBoard.ViewList.Insert(nIndex,tempOrderGroupVIEW);
    end else begin
      tabtopBoard.ViewList.Add(tempOrderGroupVIEW);    
    end;
    ReOrderViews;
    }
...
begin
  ...
  try
    ...
    tempOrderGroupVIEW.Images := PngImageCollection;
    tabtopBoard.ViewList.Remove(sourceVIEW);
    nIndex := tabtopBoard.ViewList.IndexOf(destOrderView);
    tabtopBoard.ViewList.Insert(nIndex,tempOrderGroupVIEW);
    ReOrderViews;
    }
...
begin
  //接口有BUG 会把全车间人员都转换为空，旧的BPL代码也有此问题，逻辑不清梦，现在应该从没有用过
//  if TBox('确定全部转换吗?') then
//  begin
//    m_RsLCNameBoardEx.Trainman.SetAllStateToNull(m_strWorkShopGUID);
//    TBox('转换成功');
//    InitJiaoluTab;
//  end;
end;
...
begin
  ...
  //获取所有的车站
  if not RsLCBaseDict.LCStation.GetStationsOfJiaoJu(trainJiaoluGUID,StationArray,ErrorInfo) then
  begin
    Box(ErrorInfo);
    Exit;
  end;
...
procedure TfrmNameBoardManage.mniNamedViewKeyTrainmanClick(Sender: TObject);
var
  view : TView;
  tmView : TTrainmanView;
begin
  view := m_SelectView;
  tmView := TTrainmanView(m_SelectView);
  if (view = nil) or (view.ClassName <> TTrainmanView.ClassName) then
  begin
    Box('请先指定人员信息');
    exit;
  end;
  if tmView.Trainman.strTrainmanGUID = '' then Exit;

  TFrmKeyTrainmanView.View(tmView.Trainman.strTrainmanNumber);
end;

procedure TfrmNameBoardManage.mniNamedViewFixedGroupClick(Sender: TObject);
var
  view : TView;
  tmView : TTrainmanView;
begin
  ...
  TFrmFixedGroupView.View(tmView.Trainman.strTrainmanNumber);
end;
...
begin
  ...
  //sourceIndex := TOrderGroupView(TrainmanView.Parent).Childs.IndexOf(TrainmanView) + 1;
  ...
  begin
    ...
    //sourceIndex := TOrderGroupView(Sender.Parent).Childs.IndexOf(Sender) + 1;
  end;
...
begin
  ...
  begin
    ...
    if TTrainmanView(View).Trainman.strTrainmanGUID <> '' then
      ;//miDeleteNamedTrainman.Enabled := true;
    exit;
  end;
end;
...
begin
  ...
  //sourceIndex := TOrderGroupInTrainView(TrainmanView.Parent).Childs.IndexOf(TrainmanView) + 1;
...
begin
  ...
  //sourceIndex := TNamedGroupView(TrainmanView.Parent).Childs.IndexOf(TrainmanView) + 1;
  ...
  begin
    ...
    //sourceIndex := TNamedGroupView(Sender.Parent).Childs.IndexOf(Sender) + 1;
  end;
