...
uses
  ...
  uKDXKInterfaceComponent,superobject,uGlobalDM,uTFSystem,uTrainPlan,
...
begin
  ...
  strList.LoadFromFile(GlobalDM.AppPath +DDMLFile_DIRName + DDMLFILE_VALIDTIME);
...
begin
  ...
  m_webWriteCard :=TRSLCWriteCardSection.Create(GlobalDM.GetWebApiHost,
      GlobalDM.SiteInfo.strSiteIP,GlobalDM.SiteInfo.strSiteGUID);
  m_DlDownload:=TDDMLDownload.Create;
  strFilePathName := GlobalDM.AppPath +DDMLFile_DIRName + 'Ickddds.mdb';
...
begin
  ...
  m_KDXKInterfaceComponent.KDXKDataPath := GlobalDM.AppPath + DDMLFile_DIRName ;
...
var
  ...
  bUpdated:Boolean;
begin
  lblDLUpdateTime.Caption := FormatDateTime('yyyy-mm-dd HH:nn:ss',GlobalDM.dtDLRARUpdate);
